...
## The Layers control box now collapses when not hovered over!
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView( 10.1991, 56.158075, zoom = 12) %>% 
  addTiles()
...
saveWidget(DANmapTaskTwo, "DANmapTaskTwo.html", selfcontained = TRUE)
...
saveWidget(DANmapCluster, "DANmapTaskThree.html", selfcontained = TRUE)
...
# The clustered map is kind of like distant reading, though instead of showing semantic themes and key words, this becomes a heatmap of key areas. 
# An obvious one is Aarhus, where most pins are. Generally, Jutland is disproportionately populated by our pins - as most of us are from Jutland.
# As the clustered map unclusters on zoom, the "close reading" is also possible through it, but with the unclustered map, precise locations are more intuitively seen.
...
saveWidget(DANmapTaskFive, "DANmapTaskFive.html", selfcontained = TRUE)
